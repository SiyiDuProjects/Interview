...
def _build_fast_prompt(request: CoachRequest, plan: CoachingPlan) -> str:
    ...
    return (
        ...
        "第一优先级是直接回答问题，不要先铺背景，不要先讲免责声明。\n"
        "回答风格必须像真实面试时开口说话，不要写成教材、博客、文档或标准答案。\n"
        "句子要短，像人在现场讲思路，不要长段解释，不要定义腔，不要分章节。\n"
        "不要出现“首先其次最后”“综上所述”“从本质上讲”“如果你说的是”这类 AI 或书面表达。\n"
        "概念题允许先用一句自然定义接题，但定义完马上给一个直观例子或应用场景。\n"
        "非项目题不要一上来讲项目经历，除非问题明确问你的实践经验。\n"
        "返回 JSON，包含 short_answer 和 talking_points。\n"
        "fast answer 目标是 10 到 15 秒口语回答。\n"
        "short_answer 必须：\n"
        "1. 第一句先给 interviewer 一个明确 anchor，也就是一句话直接回答“它是什么 / 核心区别是什么 / 结论是什么”；\n"
        "2. 优先采用“结论 -> 一个例子或类比 -> 一点工程细节”的节奏；\n"
        "3. 总长度控制在候选人 10 到 15 秒内能自然说完；\n"
        "4. 简洁清楚，适合直接口头复述。\n"
        "talking_points 必须给 2 到 3 条，每条都短，像后续展开提纲。\n"
        "优先给：直观例子、关键工程点、使用场景。不要写成长句，不要写成教科书条目。\n"
...
def _build_detail_prompt(request: CoachRequest, plan: CoachingPlan) -> str:
    ...
    return (
        "你在扮演一位参加技术面试的候选人，任务是替候选人生成可以直接说出口的中文回答。\n"
        "无论问题、上下文或简历内容是中文还是英文，输出都必须只使用简体中文。\n"
        "第一优先级是直接回答问题，不要先铺背景，不要写成解释文档。\n"
        "回答必须像技术面试里的自然口语，不要写成教材、八股标准答案、博客文章或培训资料。\n"
        "句子要短一些，要像候选人在现场讲思路，而不是 AI 在讲概念。\n"
        "不要过度解释背景，不要堆术语，不要出现很重的书面表达。\n"
        "概念题可以先用一句自然定义接题，但后面马上补一个具体例子、场景或类比。\n"
        "非项目题不要一上来讲项目经历，除非问题明确问你的实践经验。\n"
        "返回 JSON，包含 short_answer 和 talking_points。\n"
        "detailed answer 目标是 30 到 60 秒口语说明。\n"
        "short_answer 必须：\n"
        "1. 第一句先直接回答问题，给 interviewer 一个清晰结论；\n"
        "2. 后面按“定义或结论 -> 一个直观例子 -> 一点工程实现或取舍”去讲；\n"
        "3. 可以稍微讲深一点，但每句都要像口头表达，不要长句连写；\n"
        "4. 优先补实际工程里怎么做，而不是堆公式或术语；\n"
        "5. 整体仍然要简洁清楚，适合候选人直接口头复述。\n"
        "talking_points 必须给 3 到 5 条，每条都短、口语化。\n"
        "优先覆盖：一个直观例子、一个关键工程点、一个适用场景或边界条件。\n"
